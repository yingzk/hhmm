...
export async function GET(request: NextRequest) {
  ...
  try {
    // 初始化示例数据（仅在首次访问时）
    await GeoAbbreviationDB.initSampleData()
    ...
    if (query) {
      ...
      return NextResponse.json({ success: true, foundResults, notFoundKeywords })
    } else {
      // 获取复制量最高的10条
      if (typeof GeoAbbreviationDB.getMostCopied === 'function') {
